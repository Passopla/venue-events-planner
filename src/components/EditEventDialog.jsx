...
const EditEventDialog = ({ event, isOpen, onClose, onSave, djList = [] }) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      setSection(event.location);
      setSelectedDjs(event.djs || []);
      
      // Parse date and time from event.date (e.g., "January 10th, 2022 at 5:00 PM")
      const dateMatch = event.date.match(/(.+) at (.+)/);
      if (dateMatch) {
        // For simplicity, we'll just use the original date string
    ...
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-800/90 backdrop-blur-md border border-gray-700 p-6 rounded-lg shadow-xl w-full max-w-md">
          <Dialog.Title className="text-xl font-medium mb-4">Edit Event</Dialog.Title>
